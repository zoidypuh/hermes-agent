    ...
    try:
        with open(os.devnull, "w", encoding="utf-8") as _devnull, \
            ...
            _review_runtime, _review_uses_aux_runtime = (
                agent._background_review_runtime(agent._current_main_runtime())
            )
            ...
            # Inherit the parent's cached system prompt only when the review
            # fork uses the parent runtime. A configured auxiliary runtime is
            # meant to be cheap and small, so let it build its own reduced
            # prompt with skip_context_files=True.
