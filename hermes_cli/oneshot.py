...
def _parse_service_tier_config(raw: object) -> str | None:
    """Normalize the CLI's configured fast/priority mode for one-shot runs."""
...
def _request_options_for_oneshot(
    cfg: dict,
    model: str,
    runtime: dict,
) -> tuple[dict | None, str | None, dict | None]:
    """Resolve request options that interactive CLI startup normally wires."""
    ...
    return reasoning_config, service_tier, request_overrides or None
...
def _run_agent(
    ...
    reasoning_config, service_tier, request_overrides = _request_options_for_oneshot(
        cfg,
        effective_model,
        runtime,
    )
